...
module.exports = {
  ...
  onChat: async function ({ api, event, args, getLang, message }) {
    try {
      ...
      const response = await axios.get(`https://itsaryan.onrender.com/api/gpt?prompt=${encodeURIComponent(prompt)}&model=gpt-4`);

      if (response.status !== 200 || !response.data || !response.data.original) {
        throw new Error('Invalid or missing response from API');
      }

      const messageText = response.data.original; 
