...
${global.GoatBot.config.prefix}xl a cat --ar=2:3`);let o="1:1",s=a.findIndex(e=>e.startsWith("--ar="));-1!==s&&(o=a[s].split("=")[1],a.splice(s,1)),t.setMessageReaction("⏳",n.messageID,()=>{},!0);let i=new Date().getTime();e.reply("L'image demandé est en cours de génération \n By Riaz...",async(r,s)=>{try{let g=a.join(" "),l=`&ratio=${o}`,m=`gen?prompt=${encodeURIComponent(g)}${l}`,p=`https://team-calyx.onrender.com/${m}`,c=await global.utils.getStreamFromURL(p),y=new Date().getTime();e.reply({body:`Voici ce que j'ai pu faire 🖼️
